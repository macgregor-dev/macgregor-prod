...
(function(){
  ...
  function ($rootScope, $scope, mapsService, currentAuth, $mdSidenav, $mdBottomSheet, $log, $q, $routeParams, $firebaseObject, $location, $mdDialog, authService, $filter, $timeout) {
    ...
    authService.loadUser().$loaded().then(function() {
      ...
      $scope.userCanExport = (fullName == "Shilo Banihit");
    });
    ...
    $scope.toggleSelecting = function() {
      $scope.isSelecting = !$scope.isSelecting;
    ...
    $scope.clearSelected = function() {
      for (var m=0; m < $scope.mapObj.addresses.length; m++) {
        $scope.mapObj.addresses[m].selected = false;
      }
    };

    $scope.moveMultiAdds = function() {
      var adds = [];
      for (var m=0; m < $scope.mapObj.addresses.length; m++) {
        if ($scope.mapObj.addresses[m].selected) {
          adds.push($scope.mapObj.addresses[m]);
        }
      }
      var procFn = function() {
        var addr = adds.pop();
        if (addr != null) {
          $scope.mapSearchRes.mapAddress = addr;
          console.log("Moved: ");
          console.log(addr);
          $scope.moveAddToMap(false, addr.addId, procFn);
        } else {
      ...
      $mdDialog.show(confirm).then(function() {
        if (addr == null) {
          ...
          var procFn = function() {
            var addr = adds.pop();
            if (addr != null) {
              mapsService.deleteAdd($scope.mapObj, addr, procFn, true);
            } else {
              $scope.isSelecting = false;
              $scope.loadPartial();
            }
    ...
    $scope.moveAddToMap = function(closeWhenDone=true, addId=null, cb=null) {
      if ($scope.mapSearchRes.found == true) {
        if ($scope.mapSearchRes.mapAddress == null) {
          console.log("Moving multi...");
          $scope.moveMultiAdds();
          return;
        } else {
          addId = $scope.mapSearchRes.mapAddress.addId;
        }
        mapsService.moveAddToMap(addId, $scope.mapSearchRes.target.value, $scope.mapSearchRes.target.key, function() {
